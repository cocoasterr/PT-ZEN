...
func (r *Repository) Create(ctx context.Context, trx *sql.Tx, model map[string]interface{}) error {
...
func (r *Repository) Index(ctx context.Context, page, limit int) ([]map[string]interface{}, interface{}, error) {
	...
	var total int
	query_total := fmt.Sprintf("SELECT COUNT(id) FROM %s WHERE deleted_at IS NULL", tbName)
...
func (r *Repository) FindBy(ctx context.Context, key string, value interface{}) ([]map[string]interface{}, error) {
	...
	query := fmt.Sprintf("SELECT * FROM %s WHERE %s = %v AND deleted_at IS NULL", tbName, key, value)
	rows, _ := r.Db.QueryContext(ctx, query)
	if rows == nil {
		return nil, nil
	}
...
func (r *Repository) Update(ctx context.Context, trx *sql.Tx, id string, model map[string]interface{}) error {
	tbName := r.Model.TbName()
